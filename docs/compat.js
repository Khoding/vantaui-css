...
(function () {
  ...
  /* ---------- Probe facts (the machine-verified half) ----------
     compat-facts.json is generated by scripts/probe-css.mjs from real
     rendering. We overlay it as "✓ verified" markers on the catalog's
     combines/conflicts so the page shows what is proven vs hand-asserted.
     Absent (e.g. opened over file://) the table still works unannotated. */
  var FACTS = null;

  function loadFacts() {
    return fetch('compat-facts.json')
      .then(function (r) {
  ...
  /* A catalogued pairing is a VERIFIED coupling when the probe saw the parent
     restyle the nested child (either direction). Returns the fact or null. */
  function combinesFact(aId, bId) {
    if (!FACTS || !FACTS.components) return null;
    var a = FACTS.components[aId];
    var b = FACTS.components[bId];
    var ab = a && a.combines && a.combines[bId];
    var ba = b && b.combines && b.combines[aId];
    if (ab && ab.verified) return ab;
    if (ba && ba.verified) return ba;
    return null;
  }

  /* A declared "x ↔ y" conflict is VERIFIED when some probe mutex group shares
     at least two of its words (the probe proved that exclusion by rendering). */
  function conflictVerified(cId, str) {
    if (!FACTS || !FACTS.components || str.indexOf('↔') === -1) return false;
    var words = str.split('↔').map(function (s) {
      return s.trim();
    }).filter(function (w) {
      return /^[\w-]+$/.test(w);
    });
    if (words.length < 2) return false;
    var groups = (FACTS.components[cId] || {}).mutexGroups || [];
    return groups.some(function (g) {
      var shared = words.filter(function (w) {
        return g.indexOf(w) !== -1;
      });
    ...
    sec.appendChild(
      el(
        ...
        'Which components nest or pair idiomatically, and the words that ' +
          ...
          'A ✓ marks pairings and conflicts verified by the CSS-truth probe ' +
          '(rendered, not hand-asserted) — hover one for the evidence.',
      ),
    );
    ...
    COMPONENTS.forEach(function (c) {
      ...
      if (partners.length) {
        partners.forEach(function (p) {
          ...
          var fact = combinesFact(c.id, p);
          if (fact) {
            a.classList.add('is-verified');
            a.dataset.tip =
              'Verified coupling: nesting restyles ' + fact.restyles.join(', ') + '.';
          }
          combo.appendChild(a);
        });
      } else {
      ...
      if ((c.conflicts || []).length) {
        c.conflicts.forEach(function (x) {
          var chip = el('span', 'compat-chip compat-chip--conflict', esc(x));
          if (conflictVerified(c.id, x)) {
            chip.classList.add('is-verified');
            chip.dataset.tip = 'Mutual exclusion verified by the CSS-truth probe.';
          }
  ...
  function boot() {
    ...
    if (!main) return;
    // Load the probe facts first so combines/conflicts can be marked verified;
    // null (e.g. file://) just means no markers, the catalog still renders.
